...
export const listProps = {
    ...
    beforeChange: {
        type: Function as PropType<
            ((newVal: BaseType, oldVal: BaseType) => BaseType) | undefined
        >,
        default: null,
    },
...
export const useSelectionList = defineHooks(listProps, (props, context) => {
    ...
    const actives = reactive<BaseType[]>(
        props.modelValue != undefined ? toArray(props.modelValue) : []
    );
    ...
    watch(actives, (val) => {
        ...
        if (props.multiple) {
            value = options
                .filter((e) => val.includes(e.value.value))
                .map((e) => e.value.value);
        } else {
            value = options.find((e) => val.includes(e.value.value))?.value
                .value;
        }
    ...
    provide(InitSymbol, (option: Ref<Option>) => {
        ...
        if (
            option.value.value == props.modelValue ||
            (actives.length == 0 && defaultFirst.value)
        ) {
        ...
        return () => {
            const index = options.findIndex(
                (e) => e.value.value == option.value.value
            );
            if (index > -1) {
    ...
    function render() {
        const children = options
            .filter((e) => isActive(e.value.value))
