...
export class PolyfillFile {
    ...
    constructor(init: string, fileName: string, encoding: BufferEncoding = "binary", contentType: string = "application/octet-stream") {
...
const CONTENT_DISPOSITION_REGEX = /Content-Disposition: form-data; name="([^"]+)"(?:; filename="([^"]+)")?/
...
export function decodeFormData(data: string | Buffer | Uint8Array | FormData, boundary: string) {
    if (isFormData(data)) {
    ...
    for (let i = 0; i < chunks.length; i++) {
        ...
        if (part.fileName) {
            body[part.name] = new PolyfillFile(part.value, part.fileName, part.encoding, part.contentType)
        } else {
